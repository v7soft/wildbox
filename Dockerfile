FROM ubuntu
RUN sed -i 's/archive.ubu/ru.archive.ubu/' /etc/apt/sources.list 
RUN apt-get update ; \
...
echo -en "NO\n.\n.\nDovecot mail server\nlocalhost\n\localhost\no@localhost\n"|openssl req -newkey rsa:2048  -x509 -days 3652.5 -nodes      -rand /dev/urandom -out /etc/dovecot/dovecot.pem -keyout /etc/dovecot/private/dovecot.pem ;\
useradd wildbox ;\
